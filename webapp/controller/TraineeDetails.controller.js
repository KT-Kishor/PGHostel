sap.ui.define(["./BaseController", "../utils/validation", "sap/ui/model/json/JSONModel", "sap/m/MessageToast", "../model/formatter", "../utils/CommonJsPDF"], function (BaseController, utils, JSONModel, MessageToast, Formatter, jsPDF) {
  ...
  return BaseController.extend("sap.kt.com.minihrsolution.controller.TraineeDetails", {
    ...
    TD_validateStep: function () {
      ...
      if (bAllFieldsFilled) {
        ...
        let bValid =
          ...
          // The following line is replaced with our logic to validate the mobile number
          this.getView().byId("TD_id_Mobile").getValue().length <= this.getView().byId("TD_id_Mobile").getMaxLength() &&
        ...
        // If not all fields are filled, the step is not validated
    ...
    TD_STDcode: function (oEvent) {
      utils._LCstrictValidationComboBox(oEvent);
      const sCountryKey = oEvent.getSource().getSelectedKey();
      ...
      const sCountryCode = this.getView().getModel("oTraineeDetails").getProperty("/countryCode").toUpperCase();
      const oMatchingEntry = aCodeData.find((entry) => entry.country_code.toUpperCase() === sCountryCode);
      this.TD_validateMobile({ getSource: () => oMobileInput });
      if (this.sArgPara === "CreateTraineeFlag") {
    ...
    TD_onChangeCountry: function (oEvent) {
      ...
      this.onCountryChange(oEvent, {
        stdCodeCombo: "TD_id_STDCode",
        baseLocationCombo: "TD_id_Location",
        branchInput: "TD_id_BranchCode",
        mobileInput: "TD_id_Mobile",
      });
      this._setMobileMaxLength();
      const oMobileInput = this.getView().byId("TD_id_Mobile");
      oMobileInput.setValue("");
    ...
    TD_onSubmitData: function (oEvent) {
      try {
        ...
        var isValid =
          ...
          this.TD_validateMobile({ getSource: () => this.getView().byId("TD_id_Mobile") }) &&
          utils._LCstrictValidationComboBox(this.getView().byId("TD_id_STDCode"), "ID");
