sap.ui.define([
    ...
    function (BaseController, JSONModel, MessageToast, utils, Currency, Formatter) {
        "use strict";
        return BaseController.extend("sap.kt.com.minihrsolution.controller.CreditNoteDetails", {
            Formatter: Formatter,
            onInit: function () {
                this.getRouter().getRoute("RouteCreditNoteDetails").attachMatched(this._onRouteMatched, this);
            },
            ...
            _onRouteMatched: async function (oEvent) {
                ...
                this._makeDatePickersReadOnly(["CCI_id_Invoice", "CCI_id_Payby", "CCI_id_NavInvoice", "CCI_id_NavPayby", "CCI_id_Status"]);
                ...
                this.byId("CCI_id_AddCustComboBox").setValueState("None");
                this.byId("CCI_id_InvoiceDesc").setValueState("None");
                this.byId("CCI_id_SowPO").setValueState("None");
                this.byId("CCI_id_ConversionRate").setValueState("None");
                this.byId("CCI_id_InputMailID").setValueState("None");
                this.byId("CCI_id_IncomeTaxPercentage").setValueState("None");
                this.byId("CCI_id_SowDetails").setValueState("None");
                this.byId("CCI_id_CurrencySelect").setEditable(true);
                this._fetchCommonData("CompanyInvoice", "CompanyInvoiceData");
                this._fetchCommonData("CreditNoteItem", "CreditNoteItemData");
                ...
                oView.setModel(new JSONModel({
                    CustomerName: "", CCInvNo: "", InvoiceDate: "", Name: "", PAN: "", GST: "", Address: "", MailID: "", MobileNo: "",
                    SOWDetails: "", Type: "", InvoiceDescription: "", Currency: "INR", PayByDate: "", POSOW: "", Status: "Submitted",
                    SubTotalNotGST: "0", SubTotalInGST: "0", CreditNote: "0", LUT: "", IncomePerc: "10", CompanyCode: ""
                }), "SelectedCustomerModel");

                this.SelectedCustomerModel = oView.getModel("SelectedCustomerModel");

                oView.setModel(new JSONModel({
                    results: [], CCInvNo: this.newID, IndexNo: "", ItemID: "", UnitText: "", Particulars: "", SAC: "", Rate: "", Currency: "INR",
                    Total: "", gstAmount: "", TotalAmount: "", subTotal: ""
                }), "FilteredSOWModel");

                oView.setModel(new JSONModel({
                    createVisi: true, editVisi: false, editable: true, igstVisi: false, gstVisiable: false,
                    flexVisiable: false, CInvoice: false, addInvBtn: true, CreditNote: true, merge: false, GST: true, payByDate: false, isCredit: true ,
                ...
                this.byId("CCI_id_TableInvoiceItem").setMode("Delete");

                this.byId("CreditNote").setEnabled(true);
                this.Update = false;
                if (sArg === "X") return;
                ...
                this.visiablityPlay.setProperty("/CreditNote", false);
                this.visiablityPlay.setProperty("/MultiEmail", false);
                this.byId("CCI_id_TableInvoiceItem").setMode("None");
                this.byId("CCI_id_CurrencySelect").setEditable(false);
                ...
                try {
                    const oData = await this.ajaxReadWithJQuery("CreditNoteItem", { CCInvNo: this.decodedPath });
                    this.Update = true;
                    if (!oData.success) throw new Error("Invalid data structure");

                    var oHeader = oData.data.CreditNote?.[0] || {};
                    this.byId("CCI_id_Payby").setMinDate(new Date(oHeader.InvoiceDate));
                    this.byId("CCI_id_NavPayby").setMinDate(new Date(oHeader.InvoiceDate));
                    ...
                    const aItems = oData.data.CreditNoteItem.map((item, index) => ({ ...item, IndexNo: index + 1 }));
                    oView.setModel(new JSONModel({ CompanyInvoiceItem: aItems }), "CompanyInvoiceItemModel");

                    const { IGST = "0", SGST = "0", CGST = "0", Value, Currency, Status, CCInvNo } = oHeader;
                    ...
                    if (Currency !== "INR") {
                        ...
                        this.byId("CCI_id_SAC")?.setVisible(false);
                        this.byId("CCI_id_GSTCalculation")?.setVisible(false);
                        this.visiablityPlay.setProperty("/TDS", false);
                    } else {
                        this.visiablityPlay.setProperty("/GST", true);
                        this.byId("CCI_id_SAC")?.setVisible(true);
                        this.byId("CCI_id_GSTCalculation")?.setVisible(true);
                    ...
                    // this.Readcall("CreditNotePaymentDetail", { CCInvNo: this.decodedPath })
                } catch (error) {
            ...
            onChangeAddCustomer: async function (oEvent) {
                ...
                this.byId("CCI_id_SowDetails").setValue("").setSelectedKey("")
                this.SelectedCustomerModel.refresh(true)
                try {
                    const oAllInvoiceModel = this.getView().getModel("CompanyInvoiceData");
                    const aAllInvoices = oAllInvoiceModel ? oAllInvoiceModel.getData() : [];
                    const sSelectedCustomerName = SelectedData.companyName;
                    const aFilteredInvoices = aAllInvoices.filter(inv => inv.CustomerName === sSelectedCustomerName);
                    const oFilteredModel = new sap.ui.model.json.JSONModel({ InvoiceList: aFilteredInvoices });
                    this.getView().setModel(oFilteredModel, "FilteredInvoiceModel");
                } catch (err) {
                    this.closeBusyDialog();
            ...
            onChangeInvoiceDate: async function (oEvent) {
                if (oEvent) {
                    ...
                    this.byId("CCI_id_Payby").setMinDate(new Date(oEvent.getSource().getValue().split('/').reverse().join('-')));
                    this.byId("CCI_id_NavPayby").setMinDate(new Date(oEvent.getSource().getValue().split('/').reverse().join('-')));
                }
                this.getBusyDialog();
            ...
            OnChangeSowDetails: async function (oEvent) {
                ...
                try {
                    ...
                    this.byId("CCI_id_SAC").setVisible(isINR);
                    this.byId("CCI_id_GSTCalculation").setVisible(isINR);
            ...
            CID_onPressAddInvoiceItems: function () {
                ...
                const oSelectedCustomer = oView.getModel("SelectedCustomerModel").getData();
                const sSelectedInvoiceNo = oSelectedCustomer.InvNo; // <-- from top ComboBox

                let oData = oItemModel.getProperty("/CompanyInvoiceItem") || [];
                const currency = this.byId("CCI_id_CurrencySelect").getValue();

                const newItem = {
                    IndexNo: oData.length ? oData[oData.length - 1].IndexNo + 1 : 1,
                    InvNo: sSelectedInvoiceNo || "", 
                    Particulars: "",
                    SAC: "998314",
                    GSTCalculation: (currency === "INR") ? "YES" : "",
                    Unit: "1",
            ...
            onChangeInvoiceNo: function (oEvent) {
                const oView = this.getView();
                const sSelectedKey = oEvent.getSource().getValue();
                const oSelectedCustomerModel = oView.getModel("SelectedCustomerModel");
                
                oSelectedCustomerModel.setProperty("/InvNo", sSelectedKey);
                oSelectedCustomerModel.refresh(true);
            },
            ...
            SubmitPayload: async function (sMode) {
                ...
                const oPayload = {
                    ...
                    CreditNote: (parseFloat(oSelectedCustomerModel.CreditNote) || 0),
                };
                ...
                const aItems = aItemsRaw.map(item => {
                    const itemData = {
                        CCInvNo: oSelectedCustomerModel.CCInvNo || "-",
                        InvNo:oSelectedCustomerModel.InvNo || "-",
                        SAC: item.SAC,
                        Unit: item.Unit,
            ...
            CID_onPressSubmit: async function (oEvent) {
                try {
                    ...
                    const bMandatoryValid =
                        //  utils._LCvalidateMandatoryField(this.byId("CCI_id_CompanyCode"), "ID") &&
                        utils._LCvalidateMandatoryField(this.byId("CCI_id_AddCustComboBox"), "ID") &&
                        utils._LCvalidateDate(this.byId("CCI_id_Invoice"), "ID") &&
                        // utils._LCvalidateDate(this.byId("CCI_id_Payby"), "ID") &&
                        // utils._LCvalidateMandatoryField(this.byId("CCI_id_InvoiceDesc"), "ID") &&
                        // utils._LCvalidateMandatoryField(this.byId("CCI_id_SowPO"), "ID") &&
                        utils._LCvalidateMandatoryField(this.byId("CCI_id_CurrencySelect"), "ID");
                    const bTDSValid = oModel.Currency === "INR" ? utils._LCvalidateVariablePay(this.byId("CCI_id_IncomeTaxPercentage"), "ID") : true;
                    const bConversionRateValid = oModel.Currency !== "INR" ? utils._LCvalidateAmount(this.byId("CCI_id_ConversionRate"), "ID") : true;
                    ...
                    try {
                        var response = await that.ajaxCreateWithJQuery("CreditNote", { data: oPayload.payload, Items: oPayload.items });
                        const oSelectedCustomerModel = that.getView().getModel("SelectedCustomerModel");
                        oSelectedCustomerModel.setProperty("/CCInvNo", response.InvoiceNo);
                        ...
                        var oDialog = new sap.m.Dialog({
                            ...
                            content: new sap.m.Text({
                                text: this.i18nModel.getText("creditCreatemsg")
                            }),
                            beginButton: new sap.m.Button({
                                ...
                                press: function () {
                                    oDialog.close();
                                    that.getRouter().navTo("RouteCreditNote");
                                }
                            }),
            ...
            CID_onPressEdit: function () {
                ...
                if (isEditMode) {
                    ...
                    this.visiablityPlay.setProperty("/CreditNote", true);
                    this.visiablityPlay.setProperty("/merge", false);
                    this.visiablityPlay.setProperty("/MultiEmail", false);
                    this.visiablityPlay.setProperty("/payByDate", false);
                }
            ...
            onPressUpdateInvoice: async function () {
                try {
                    var oModel = this.getView().getModel("FilteredSOWModel").getData();
                    const bIsValid =
                        // utils._LCvalidateMandatoryField(this.byId("CCI_id_InvoiceDesc"), "ID") && this.mobileNo &&
                        // utils._LCvalidateMandatoryField(this.byId("CCI_id_SowPO"), "ID") &&
                        utils._LCvalidateEmail(this.byId("CCI_id_InputMailID"), "ID") &&
                        (!!this.Discount && !!this.RateUnit && !!this.Particulars) &&
                        (oModel.Currency === "INR" ? utils._LCvalidateVariablePay(this.byId("CCI_id_IncomeTaxPercentage"), "ID") : utils._LCvalidateAmount(this.byId("CCI_id_ConversionRate"), "ID"));
                    ...
                    try {
                        await this.ajaxUpdateWithJQuery("CreditNote", { data: oPayload.payload, filtres: oPayload.filters, Items: oPayload.items });
                        this.visiablityPlay.setProperty("/editable", false);
                        ...
                        MessageToast.show(this.i18nModel.getText("creditUpdateMsg"));
                        this.byId("CreditNote").setEnabled(true);
                        this.closeBusyDialog();
                    } catch (error) {
                        this.closeBusyDialog();
                ...
                if (status === "Payment Received" || status === "Payment Partially" || status === "Open") {
                    ...
                    if (!that.oDialog) {
                        that.oDialog = sap.ui.core.Fragment.load({
                            name: "sap.kt.com.minihrsolution.fragment.CreditNote",
                            controller: that
            ...
            modelFunction: function () {
                ...
                var oModel = new JSONModel({
                    CCInvNo: oNavigationModel.CCInvNo,
                    TransactionId: "",
                    ReceivedDate: "",
                    ReceivedAmount: "",
            ...
            onChangeReceivedAmount: function (oEvent) {
                ...
                if (oEvent) {
                    ...
                    if (enteredAmount === dueAmount) {
                        sap.ui.getCore().byId("CC_id_ReceivedAmount").setValueState("None");
                    } else if (enteredAmount > dueAmount) {
                        this.ResivedAmount = false;
                        sap.ui.getCore().byId("CC_id_ReceivedAmount").setValueState("Error");
                        sap.ui.getCore().byId("CC_id_ReceivedAmount").setValueStateText(this.i18nModel.getText("invoiceRecievedAmountMessage"));
                    } else {
                        sap.ui.getCore().byId("CC_id_ReceivedAmount").setValueState("None");
                        this.ResivedAmount = true;
            ...
            onChangeReceivedTDS: async function (oEvent) {
                var oInput = sap.ui.getCore().byId("CC_id_ReceivedTDS");
            ...
            onPressInvClose: function () {
                sap.ui.getCore().byId("CC_id_TransactionID").setValueState("None");
                sap.ui.getCore().byId("CC_id_ReceivedAmount").setValueState("None");
                sap.ui.getCore().byId("CC_id_ReceivedTDS").setValueState("None");
                sap.ui.getCore().byId("CC_id_FrgConvertionRate").setValueState("None");
            ...
            onChangePaymentRecived: async function () {
                ...
                const isMandatoryValid =
                    utils._LCvalidateMandatoryField(sap.ui.getCore().byId("CC_id_TransactionID"), "ID") &&
                    utils._LCvalidateDate(sap.ui.getCore().byId("CC_id_ReceivedDate"), "ID");

                let isCurrencyValid = true;
                if (paymentModel.Currency !== "INR") {
                    isCurrencyValid = utils._LCvalidateAmount(sap.ui.getCore().byId("CC_id_FrgConvertionRate"), "ID");
                } else {
                ...
                if (isReceivedAmountInvalid) {
                    sap.ui.getCore().byId("CC_id_ReceivedAmount").setValueState("Error").setValueStateText(this.i18nModel.getText("invoiceRecievedAmountMessage"));
                } else {
                    sap.ui.getCore().byId("CC_id_ReceivedAmount").setValueState("None");
                }
                if (paymentModel.Currency === "INR") {
                    if (isReceivedTDSInvalid) {
                        sap.ui.getCore().byId("CC_id_ReceivedTDS").setValueState("Error").setValueStateText(this.i18nModel.getText("tdsAmountError"));
                    } else {
                        sap.ui.getCore().byId("CC_id_ReceivedTDS").setValueState("None");
                    }
                }
                ...
                this.getBusyDialog();
                const jsonData = {
                    CCInvNo: String(paymentModel.CCInvNo),
                ...
                try {
                    ...
                    if (oData && oData.success) {
                        this.oDialog.close();
                        this.Readcall("CreditNotePaymentDetail", { CCInvNo: this.decodedPath });
                        this.Readcall("CreditNote", { CCInvNo: this.decodedPath });
            ...
            CID_onPressDelete: function (oEvent) {
                ...
                if (oContext.getObject().IndexNo === this.IndexNo) {
                    this.byId("CreditNote").setEnabled(true);
                }
                ...
                if (oContext.getObject().ItemID) {
                    this.showConfirmationDialog(
                        ...
                        function () {
                            that.getBusyDialog();
                            that.ajaxDeleteWithJQuery("/CreditNoteItem", {
            ...
            Mail_onSendEmail: function () {
                try {
                    ...
                    var oPayload = {
                        "CCInvNo": SelectedModel.getData().CCInvNo,
            ...
            CID_onPressGeneratePdf: async function () {
                ...
                const type = this.getView().byId("CCI_id_Type").getText();
                let filter = { companyCode: oModel.CompanyCode }
                try {
                    ...
                    doc.text("Credit - Memo", pageWidth - 18, headerMargin, {
                        align: "right"
                    });
                    ...
                    if (data.Currency !== "USD") {
                        ...
                        if (data.Currency === "INR" && (oModel.Type === "CGST/SGST" || type.split(" ")[0] === "CGST/SGST") && cgstValue > 0) {
                            summaryBody.push([`Credit CGST ${cgstPercentage} :`, Formatter.fromatNumber(cgstValue.toFixed(2))]);
                            summaryBody.push([`Credit SGST ${sgstPercentage} :`, Formatter.fromatNumber(sgstValue.toFixed(2))]);
                        } else if (data.Currency === "INR" && (oModel.Type === "IGST" || type.split(" ")[0] === "IGST") &&
                            igstValue > 0) {
                            summaryBody.push([`Credit IGST ${igstPercentage} :`, Formatter.fromatNumber(igstValue.toFixed(2))]);
                        }
                    }

                    if (data.RoundOf && data.RoundOf !== "0") {
                        summaryBody.push([`Credit Round Off (${data.Currency}) :`, data.RoundOf]);
                    }
                    ...
                    const logoXPosition = rightEdgeX - signatureWidth + 12; // align to rightEdge
