    ...
    return BaseController.extend("sap.ui.com.project1.controller.ManageProfile", {
        ...
        _normalizeBookingData: function (booking, mBranchGSTData) {
            ...
            return {
                ...
                RoomNo: String(booking.RoomNo || "").trim(),
        ...
        onEditMemberFromDialog: async function (oEvent) {
            ...
            const sEditUserID = oData.UserID ||
        ...
        _previewDocument: async function (oDoc) {
            ...
            function normalizeBase64(sValue) {
            ...
            function autoDecodeBase64(sValue) {
                if (!sValue) {
                    return "";
                }
                ...
                for (let i = 0; i < 5; i++) {
                    try {
                        if (
                            sDecoded.startsWith("iVB") ||      // PNG
                            sDecoded.startsWith("/9j") ||      // JPG / JPEG
                            sDecoded.startsWith("JVBER") ||    // PDF
                            sDecoded.startsWith("UklGR")       // WEBP
                        ) {
                            return sDecoded;
                        }
            ...
            if (sMimeType === "application/pdf") {
                ...
                try {
                    ...
                    console.error("[Preview] PDF decode failed", {
                        error: oError?.message || oError,
                        fileName: sFileName,
                        fileType: sMimeType,
                        rawSourcePrefix: sRawSource.slice(0, 200),
                ...
                this._pdfBlobUrl = URL.createObjectURL(oBlob);

                // Mobile: directly download PDF instead of iframe preview
            ...
            ">
                <iframe
                    src="${this._pdfBlobUrl}#toolbar=0&navpanes=0&scrollbar=0"
                    style="
                        border:none;
                        width:100%;
                        height:100%;
        ...
        savepress: function () {
            ...
            if (utils._LCstrictValidationComboBox(oView.byId("idSelect"), "ID") &&
                ...
                const sUserID = oMember.UserID ||
                    (this._existingFileData && this._existingFileData.UserID) ||
                    this.getView().getModel("profileData").getProperty("/UserID") ||
                    "";

                const sDocumentID = this._mode === "UPDATE" ?
                ...
                this._uploadDocument(oPayload, sDocumentID);
        ...
        _setComplaintRoomComboData: function (sBranchCode, sSelectedRoomNo) {
            ...
            this._applyComplaintCustomerFilter(sNewRoom);
        },
        ...
        _getBookingIDsForRoom: function (sRoomNo) {
            const oProfileModel = this.getView().getModel("profileData");
            const aBookings = oProfileModel?.getProperty("/bookings") || [];
            const sRoom = String(sRoomNo || "").trim();
            const sBranch = String(this.BranchCode || "").trim();
        ...
        _syncComplaintCombo: function (sId, sKey, bLock) {
            const oCombo = this._getComplaintControl(sId);
            if (!oCombo) {
                return;
            }
        ...
        _applyComplaintCustomerFilter: function (sRoomNo) {
            const oView = this.getView();
            const oTempModel = oView.getModel("complaintTemp");
            const aAll = this._getComplaintCustomerData();
            ...
            const bRoomInData = aAll.some(function (oItem) {
            ...
            if (!sRoom) {
                ...
                // Fallback: resolve the room via the profile's own booking list
                const aRoomBookingIDs = this._getBookingIDsForRoom(sRoom);
            ...
            let oCustomerModel = oView.getModel("customerbookingdata");
            if (!oCustomerModel) {
            ...
            const aCustomers = [...new Set(aFiltered
                .map(function (oItem) { return oItem.CustomerName; })
                .filter(Boolean))];

            const aBookingIDs = [...new Set(aFiltered
                .map(function (oItem) { return oItem.BookingID; })
                .filter(Boolean))];

            let sCustomer = String(oTempModel.getProperty("/CustomerName") || "").trim();
            let sBookingID = String(oTempModel.getProperty("/BookingID") || "").trim();
            ...
            if (oComplaintData) {
                ...
                if (this.BranchCode) {
                    ...
                    this.onSearch().then(() => {
                        ...
                        // Re-apply room based filter + lock for the saved room
                        this._applyComplaintCustomerFilter(
                            oTempModel.getProperty("/RoomNo")
                        );
        ...
        onCloseComplaintDialog: function () {
            ...
            // Drop cached customer data and unlock Customer Name / Booking ID
            this._aComplaintCustomerData = [];
        ...
        onComplaintRoomChange: function (oEvent) {
            const oRoomCombo = oEvent.getSource();
            const bValidRoom = utils._LCstrictValidationComboBox(oEvent);
            const sRoomNo = bValidRoom ? oRoomCombo.getSelectedKey() : "";
            ...
            if (!sRoomNo) {
                // Invalid / cleared room → reset dependent fields
                this._clearComplaintCustomerBooking();
                return;
            }

            // Filter + lock Customer Name and Booking ID for this room
            ...
            return new Promise((resolve, reject) => {
                ...
                this.ajaxReadWithJQuery("HM_CustomerReadCall", filter)
                    ...
                    .then((oData) => {
                        ...
                        const oTempModel = this.getView().getModel("complaintTemp");
                        this._applyComplaintCustomerFilter(
                            oTempModel ? oTempModel.getProperty("/RoomNo") : ""
                        );
        ...
        HF_viewimage: async function (oEvent) {
            ...
            // Destroy old dialog every time, same as admin preview
            if (this._oPreviewDialog) {
                this._oPreviewDialog.destroy();
                this._oPreviewDialog = null;
            }

            // Load reusable preview fragment
            this._oPreviewDialog = await sap.ui.core.Fragment.load({
                id: this.getView().getId(),
                name: "sap.ui.com.project1.fragment.DocumentPreview",
                controller: this
