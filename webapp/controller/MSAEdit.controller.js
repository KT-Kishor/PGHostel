sap.ui.define(["./BaseController", "../utils/validation", "sap/ui/model/json/JSONModel", "../utils/CommonAgreementPDF", "sap/m/MessageToast", "../model/formatter"], function (BaseController, utils, JSONModel, jsPDF, MessageToast, Formatter) {
  "use strict";
  return BaseController.extend("sap.kt.com.minihrsolution.controller.MSAEdit", {
    ...
    MsaE_validateDate: function (oEvent) {
      if (oEvent.getParameter("id").split("_").pop() === "StartDate") {
        sap.ui
          .getCore()
          .byId("SOW_id_EndDate")
          .setMinDate(new Date(oEvent.getParameter("value").split("/").reverse().join("-")));
      }
    ...
    _setMobilelength: function () {
      ...
      const oMobileInput = sap.ui.getCore().byId("Msa_LC_Mobile");
      if (sCountry === "India") {
    ...
    MSA_validateMobile: function (oEventOrControl) {
      ...
      const sCountryName = this.getView().getModel("FilteredMsaModel").oData[0].Country;
    ...
    MSA_onChangeState: function (oEvent) {
      ...
      if (!oSelectedItem) {
        ...
        // filter cities
        oCityCombo.getBinding("items")?.filter([new sap.ui.model.Filter("stateName", sap.ui.model.FilterOperator.EQ, sStateName), new sap.ui.model.Filter("countryCode", sap.ui.model.FilterOperator.EQ, sCountryCode)]);
...
_applyCountryStateCityFilters: function () {
      ...
      if (selectedItem && selectedItem.SNo) {
        this.showConfirmationDialog(
          ...
          function () {
            that.getBusyDialog();
            that
              .ajaxDeleteWithJQuery("/SowDetails", {
                filters: {
    ...
    TableGetData: async function (value) {
      ...
      await this._fetchCommonData("SowDetails", "SowAllDataModel", {
        SowID: SowID,
      });
      this.closeBusyDialog();
      ...
      var FilterData = JSON.parse(JSON.stringify(SowReadModel.filter((item) => item.SowID === this.Selected.SowID)));
    ...
    Mail_onSendEmail: function () {
      try {
        ...
        this.getBusyDialog();
        this.ajaxCreateWithJQuery(this.Type === "MSA" ? "MSAEmail" : "SOWEmail", oPayload)
          .then((oData) => {
    ...
    SOW_onDeleteExtendItem: function () {
      ...
      var aIndexesToDelete = aSelectedPaths
        .map(function (path) {
    ...
    async MsaE_onPressMergeSow() {
      var tableData = this.getView()
        .getModel("SowReadModel")
        .getData()
        .filter((item) => item.SowID === this.Selected.SowID && item.Status !== "Inactive");
      ...
      await this._fetchCommonData("PDFCondition", "PDFSOWModel", {
        Type: "SOW",
      });
      oPDFModel.setProperty("/AgreementDate", Formatter.formatDate(oModel.CreateMSADate));
      oPDFModel.setProperty("/ClientCompanyName", oModel.CompanyName);
