sap.ui.define([
        ...
        return BaseController.extend("sap.kt.com.minihrsolution.controller.AssetAssignment", {
            ...
            FAA_onTypeChange: async function (oEvent) {
                ...
                sap.ui.getCore().byId("FAA_id_Model").setBusy(true);
                            //   Set BranchName for IT Consultant or other roles if needed
    if (this.oLoginModel.getProperty("/Role") === "IT Consultant") {
        await this._fetchCommonData("BaseLocation", "branchModel");
        let branchData = this.getOwnerComponent().getModel("branchModel").getData() || [];
        let branch = branchData.find(item => item.branchCode == this.oLoginModel.getProperty("/BranchCode"));
        if (branch) {
            this.oLoginModel.setProperty("/BranchName", branch.city);
        }
    }
                 var params;
                (this.oLoginModel.getProperty("/Role") === "IT Consultant") ? params = { PickedBranch: this.oLoginModel.getProperty("/BranchName"), ReturnBranch: this.oLoginModel.getProperty("/BranchName") } : params = {};
            ...
            AA_onSearch: function () {
                ...
                var params = {};
                // (this.oLoginModel.getProperty("/Role") === "IT Consultant") ? params = { AssignBranch: this.oLoginModel.getProperty("/BranchName"), PickedBranch: this.oLoginModel.getProperty("/BranchName") } : params = {};
                aFilterItems.forEach(function (oItem) {
                ...
                this.closeBusyDialog();
                // this.ajaxReadWithJQuery("IncomeAsset", params, ["AA_id_AssestTable"]).then((oData) => {
                //     var offerData = Array.isArray(oData.data) ? oData.data : [oData.data];
                //     var filteredData = offerData.filter(item =>
            ...
            // formatReturnDate: function (sDate) {
            //     if (!sDate) {
            //         return "";
            //     }
            ...
            //     // Check if it's 30/11/1899
            //     if (
            //         oDate.getDate() === 30 &&
            //         oDate.getMonth() === 10 && // Month is 0-indexed (10 = November)
            //         oDate.getFullYear() === 1899
            //     ) {
            //         return "";
            //     }
    ...
    // Avoid default 1899-11-30 date (often from null Date objects)
    if (!sDate || sDate === "1899-11-30T00:00:00" || sDate === "1899-11-30" || sDate.includes("1899-11-30")) {
        return "Return Date:";
    }

    // Call shared formatter safely
    const oFormatter = this.getView().getController().Formatter;
    return "Return Date: " + oFormatter.formatDate(sDate);
}
...
//             FAA_filterEmpByBranch: function () {
//     var sConsultantBranch = this.oLoginModel.getProperty("/Branch");
//     var aAllEmployees = this.getView().getModel("EmpModel").getProperty("/");
    
//     var aFiltered = aAllEmployees.filter(function (oEmp) {
//         return oEmp.Branch === sConsultantBranch;
//     });

//     // Set filtered model
//     var oFilteredEmpModel = new sap.ui.model.json.JSONModel(aFiltered);
//     this.getView().setModel(oFilteredEmpModel, "FilteredEmpModel");
// },
