    ...
    function (BaseController, JSONModel, MessageToast, utils, Currency, Formatter) {
        ...
        return BaseController.extend("sap.kt.com.minihrsolution.controller.CompanyInvoiceDetails", {
            ...
            _onRouteMatched: async function (oEvent) {
                ...
                if (!this.getView().getModel("CCMailModel")) this._fetchCommonData("EmailContent", "CCMailModel", {
                    Type: "CompanyInvoice",
                    Action: "CC"
                });
                ...
                this.getBusyDialog();
                try {
                    const oData = await this.ajaxReadWithJQuery("CompanyInvoiceItem", {
                        InvNo: this.decodedPath, Status: "Used"
                    });
                    this.Update = true;
                    ...
                    const aItems = oData.data.CompanyInvoiceItem.map((item, index) => {
                        const newItem = { ...item, IndexNo: index + 1 };
                        if (item.Status === "Used") newItem.isCredit = true;
                        return newItem;
                    });
            ...
            onChangeAddCustomer: async function (oEvent) {
                ...
                await this.ajaxCreateWithJQuery("combineSowMsa", {
                    MsaID: this.SelectKey
                }).then((oData) => {
                    if (oData.success) {
            ...
            OnChangeSowDetails: async function (oEvent) {
                ...
                try {
                    ...
                    const oData = await this.ajaxReadWithJQuery("SowDetails", {
                        MsaID: msaID,
                        SowID: sowID,
                        Status: "Active"
                    });
            ...
            totalAmountCalculation: function () {
                ...
                const aSOWDetails = oInvoiceModel.getProperty("/CompanyInvoiceItem") || [];

                let totalTaxable = 0;        // Normal taxable
                let totalNonTaxable = 0;     // Normal non-taxable
                let creditTaxable = 0;       // Credit taxable
                let creditNonTaxable = 0;    // Credit non-taxable

                const currency = oSOWModel.getProperty("/Currency");
                let gstRate = parseFloat(oCustomerModel.getProperty("/Value")) || 0;
                ...
                aSOWDetails.forEach((item) => {
                    ...
                    // --- Split normal vs credit memo ---
                    const isCredit = item.Status === "Used" || (item.Particulars && item.Particulars.toLowerCase().includes("credit"));

                    if (isCredit) {
                        if (isGSTApplicable) creditTaxable += finalAmount;
                        else creditNonTaxable += finalAmount;
                    } else {
                        if (isGSTApplicable) totalTaxable += finalAmount;
                        else totalNonTaxable += finalAmount;
                    }
                });

                // --- NORMAL INVOICE CALCULATION ---
                let gstAmount = 0, cgst = 0, sgst = 0, igst = 0;
                let subTotalGST = totalTaxable;
                let subTotalNoGST = totalNonTaxable;
                let finalNormal = subTotalGST + subTotalNoGST;

                if (currency === "INR") {
                    if (gstType === "CGST/SGST") {
                        gstAmount = (subTotalGST * gstRate) / 100;
                        cgst = gstAmount;
                        sgst = gstAmount;
                        finalNormal += (cgst + sgst);
                ...
                const roundedNormal = Math.round(finalNormal);
                const roundOffNormal = (roundedNormal - finalNormal).toFixed(2);
                ...
                if (creditTaxable > 0 && currency === "INR") {
                    if (gstType === "CGST/SGST") {
                        creditGSTAmt = (creditTaxable * gstRate) / 100;
                        creditCGST = creditGSTAmt;
                        creditSGST = creditGSTAmt;
                        creditFinal += (creditCGST + creditSGST);
                    } else if (gstType === "IGST") {
                        creditGSTAmt = (creditTaxable * gstRate) / 100;
                        creditIGST = creditGSTAmt;
                        creditFinal += creditIGST;
                    }
                }

                const roundedCredit = Math.round(creditFinal);
                const creditRoundOff = (roundedCredit - creditFinal).toFixed(2);
                const CreditRoundOff = creditRoundOff > 0 ? `+${creditRoundOff}` : creditRoundOff;
                const grandTotal = roundedNormal - roundedCredit;

                // --- Update Models ---
                oCustomerModel.setProperty("/SubTotalInGST", subTotalGST.toFixed(2));
                oCustomerModel.setProperty("/SubTotalNotGST", subTotalNoGST.toFixed(2));
                ...
                oSOWModel.setProperty("/subTotal", (subTotalGST + subTotalNoGST).toFixed(2));
                oSOWModel.setProperty("/RoundOf", RoundOff);
                oSOWModel.setProperty("/Total", grandTotal.toFixed(2));
            ...
            SubmitPayload: async function (sMode) {
                ...
                const aItems = aItemsRaw.filter(item => item.Status !== "Used").map(item => {
                    const itemData = {
            ...
            CID_onPressSubmit: async function (oEvent) {
                try {
                    ...
                    try {
                        var response = await that.ajaxCreateWithJQuery("CompanyInvoice", {
            ...
            onPressUpdateInvoice: async function () {
                try {
                    ...
                    try {
                        await this.ajaxUpdateWithJQuery("CompanyInvoice", {
                            data: oPayload.payload,
            ...
            modelFunction: function () {
                ...
                var oModel = new JSONModel({
                    ...
                    DueAmount: (this.getView().getModel("InvoicePayment").getData().length !== 0 ?
                        parseFloat(this.getView().getModel("InvoicePayment").getProperty("/AllDueAmount")) - parseFloat(ResivedTDSData || 0) : parseFloat(oNavigationModel.TotalAmount || 0) - parseFloat(oNavigationModel.IncomeTax || 0)).toFixed(2),
            ...
            CID_onPressDelete: function (oEvent) {
                ...
                if (oContext.getObject().Status === "Used") {
            ...
            CID_onPressGeneratePdf: async function () {
                ...
                try {
                    ...
                    const body = oCompanyItemModel.filter(item => item.Status !== "Used").map((item, index) => {
                        const row = [
                            index + 1,
                            item.Particulars,
                            item.Unit || "-",
