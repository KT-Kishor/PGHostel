    ...
    MessageBox
          ...
          return BaseController.extend("sap.kt.com.minihrsolution.controller.Timesheet", {
            ...
            _getStartOfWeek: function (date) {
              ...
              TS_onFillDetails: function () {
                  this.getRouter().navTo("RouteTimesheetDetails",{sPath:"Create"});
              },
  
  
              TS_onPressData: function (oEvent) {
                  const oSelectedItem = oEvent.getSource();
                  const oContext = oSelectedItem.getBindingContext("FilteredTimesheetModel");
                  const oSelectedData = oContext.getObject();
              
                  const oModel = new sap.ui.model.json.JSONModel(oSelectedData);
                  this.getOwnerComponent().setModel(oModel, "EditTimesheetModel");
              
                  this.getRouter().navTo("RouteTimesheetDetails",{sPath:"Edit"});
              },
  
  
              // TS_onPressData: function (oEvent) {
              //     const oSelectedContext = oEvent.getSource().getBindingContext("FilteredTimesheetModel");
              //     const oSelectedData = oSelectedContext.getObject();
              
              //     // Create a model to pass data
              //     const oModel = new sap.ui.model.json.JSONModel(oSelectedData);
              //     this.getOwnerComponent().setModel(oModel, "editEntryModel");
              
              //     // Update flags in viewModel
              ...
              //     oViewModel.setProperty("/isSubmitted", oSelectedData.Status === "Submitted"); // Assuming 'Status' field
              
              //     // Navigate to details page
              //     this.getOwnerComponent().getRouter().navTo("RouteTimesheetDetails",{sPath:"Edit"});
              // },
              ...
              onInitializeCalendarLegend: function () {
                ...
                // Removed _markHolidaysOnCalendar() from here
            },
          ...
          TSD_ReadTimesheetEntries: async function () {
              try {
                  ...
                  // this._fetchCommonData("EmployeeDetails", "EmployeeModel", { EmployeeID: sEmployeeID });
          
                  const result = await this.ajaxReadWithJQuery("Timesheet", {
                      EmployeeID: sEmployeeID // <-- This filters at backend (if supported)
                  });
          
                  let timesheetData = result?.data || [];
          
                  // Additional safety filter on frontend in case backend returns all data
                  timesheetData = timesheetData.filter(entry => entry.EmployeeID === sEmployeeID);
          
                  const oTimesheetModel = new sap.ui.model.json.JSONModel(timesheetData);
                  this.getView().setModel(oTimesheetModel, "TimesheetModel");
          
                  // By default, show all timesheet data (before any calendar date is selected)
                  const oFilteredModel = new sap.ui.model.json.JSONModel(timesheetData);
                  this.getView().setModel(oFilteredModel, "FilteredTimesheetModel");
          
              } catch (error) {
                  console.error("Error reading Timesheet entries:", error);
